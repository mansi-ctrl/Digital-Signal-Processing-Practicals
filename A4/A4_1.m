...
% ************************* PARAMETERS **************************%
A=input( 'Enter amplitude: ' ); % amplitude
f=input( 'Enter frequency: ' ); % signal frequency
P=input( 'Enter initial phase: ' ); % phase
fs=input( 'Enter sampling frequency: ' ); % sampling frequency
cycles=input( 'Enter the number of cycles: ' ); % number of periods/cycles
% A=5;
% f=50;
% P=0;
% fn=2*f;
...
% % fs=fn;
lower_limit=0;
upper_limit=cycles/f;
t=lower_limit:1/fs:upper_limit;
% ************************* SIGNALS **************************%
x=A*cos((2*pi*f).*t + P); % Input dicrete signal: x(n)
y_myfunc=dft(x); % DFT on x using my own function
y_inbuilt=fft(x); % DFT on x using inbuilt MATLAB function fft
...
hold on;
grid on; % make grid lines on
title('DFT of x(n)-InbuiltFunc MagnitudeSpecturm'); % title of the figure
...
hold on;
grid on; % make grid lines on
title('DFT of x(n)-OwnFunc MagnitudeSpecturm'); % title of the figure
xlabel('f'); % label for x-axis
ylabel('Magnitude'); % label for y-axis
